...
const Sidebar = () => {
 ...
 const filteredUsers = input ? users.filter((users)=>users.fullName.toLowerCase().includes(input.toLowerCase())) : users ; 
  ...
  return (
    <div className="h-full p-5 rounded-r-xl overflow-y-scroll text-white bg-[#8185B2]/10">
        ...
        {filteredUsers.map((users, index) => (
          <div
            ...
            onClick={() => {setSelectedUser(users) ;
              ...
              setUnseenMessages(prev => ({...prev , [users._id]:0}))
            }
              
             }
            className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm 
              ${selectedUser?._id === users._id ? 'bg-[#282142]/50' : ''}`}
          >
            <img
              src={users?.profilePic || assets.avatar_icon}
              alt=""
